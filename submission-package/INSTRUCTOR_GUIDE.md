...
1. **`01_Dashboard_View.png`**: Displays the glassmorphic React dashboard showing the notes currently live in the MySQL database.
2. **`02_Semantic_Search_Laravel.png`**: Demonstrates semantic search in action. When searching for `"how to build clean Laravel code"`, notes about **PHP Coding** and **Laravel Architecture** score above **75%** and rank at the top, while unrelated notes score low.
3. **`03_Semantic_Search_Pizza.png`**: Shows the reverse search query `"pizza"` where the **Italian Pizza Recipe** jumps to Rank #1 while coding notes drop to the bottom.
4. **`04_AI_Summary_Modal.png`**: Shows the summarization overlay pop-up modal displaying a concise 3-5 line summary generated by Gemini and fetched from the Redis cache.
5. **`05_Swagger_API_Explorer.png`**: Displays the interactive Swagger OpenAPI docs listing all backend endpoints.
6. **`06_PHPUnit_Test_Results.png`**: Displays the green terminal verification showing all 9 test suites passing.
